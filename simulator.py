...
filename_geojson = "sfc_tmp.geojson"
...
def animate(time):
  ...
  reward_sum = 0
  stop_reward = -0.00000000000000000001 #停止時の報酬
  ...
  for car in cars_list:
    ...
    if car.stop_flag == False:
      x_new, y_new, goal_arrived_flag = car.move(DG, edges_cars_dic, sensitivity)
    else:
      x_new = car.current_end_node[0]
      y_new = car.current_end_node[1]
      car.goal_arrived_flag = True
    ...
    if car.goal_arrived == True:
      ...
      if (index_x, index_y) not in car.experience[index_time].keys():
        car.experience[index_time][(index_x, index_y)] = {
              "reward": 0,
              "count": 0,
              "step": 0,
              "reward per step": 0,
      ...
      if (orig_index_x, orig_index_y) in car.experience[index_time].keys():
        if car.stopcount == False:
          car.experience[index_time][(orig_index_x, orig_index_y)]["step"] += car.num_of_elapsed_steps
          step = car.experience[index_time][(orig_index_x, orig_index_y)]["step"]
          reward = car.experience[index_time][(orig_index_x, orig_index_y)]["reward"]
          car.experience[index_time][(orig_index_x, orig_index_y)]["reward per step"] = reward / step
        else:
          car.experience[index_time][(orig_index_x, orig_index_y)]["step"] += 1
          step = car.experience[index_time][(orig_index_x, orig_index_y)]["step"]
          reward = car.experience[index_time][(orig_index_x, orig_index_y)]["reward"]
          car.experience[index_time][(orig_index_x, orig_index_y)]["reward per step"] = reward / step
        ...
        #乗客を拾えなかった場合、epsilon greedy で次の目的地を決める
          ...
          #現在地からの距離を考慮した、過去の経験からの目的地の設定
            ...
            if len(car.experience[index_time]) == 0: #過去の経験がない場合はランダムで選ぶ
              ...
              #停止戦略
              if "reward per step" in car.experience[index_time][(index_x, index_y)]:
                car.stopcount += 1   #とどまる続けている数
                #stop_reward = -10 #-0.0001  #停止時の報酬
                all_stop_reward = stop_reward * car.stopcount
                all_stop_reward_per_step = car.experience[index_time][(index_x, index_y)]["reward per step"] + all_stop_reward
                if max_reward_per_step > all_stop_reward_per_step:
                  car.stop_flag = False
                  car.stopcount = 0
                  print("客を探して移動")
                  ...
                  car.stop_flag = True
                  #car.goal_arrived = True
                  print("停止戦略を実行",car.stopcount)
              else:
                car.stop_flag = False
                car.stopcount = 0
                print("客を探して移動")
      ...
      while True:
        try:
          shortest_path = nx.dijkstra_path(DG, orig_node_id, dest_node_id) #最短経路を求める関数
        except (nx.NetworkXNoPath, nx.NodeNotFound):
          ride_flag = False #最短経路の計算でエラーの場合は、目的地をランダムで再設定
          dest_node_id = choose_dest_node_at_random()
          continue
        if len(shortest_path) == 1:
          if car.stop_flag == True:
            break
          dest_node_id = choose_dest_node_at_random()
          continue
        if ride_flag:
      ...
      if car.stop_flag == False:
        car_id_datas.append(car.identity)
      ...
      car.experience[index_time][(index_x, index_y)]["reward"] -= stop_reward * car.stopcount
      car.total_reward -= moving_distance / 10
      car.total_reward -= stop_reward * car.stopcount
      car.experience[index_time][(index_x, index_y)]["count"] += 1

      # create new car
      car_id = car.identity
      new_car = Car(orig_node_id, dest_node_id, shortest_path, num_of_division, car_id)
      ...
      new_car.stop_flag = car.stop_flag
      new_car.stopcount = car.stopcount
      new_car.identity = car.identity
      if new_car.stop_flag == False:
        new_car.init(DG)
        ...
        new_car.goal_arrived = car.goal_arrived
        current_end_node_id = new_car.shortest_path[ 0 ]
        new_car.current_end_node = DG.nodes[ current_end_node_id ]["pos"]
...
if __name__ == "__main__":
  ...
  while i < number_of_cars:
    ...
    car = Car(origin_node_id, destination_node_id, shortest_path, num_of_division, car_id)
  ...
  ani = FuncAnimation(fig, animate, frames=range(60000), init_func=init, blit=True, interval= 50)
  #ani.save(str(epsilon) + "sfc-small.gif", writer=PillowWriter())
  #ani.save(str(epsilon) + "EntireSanFrancisco.mp4", writer="ffmpeg")
  ani.save(str(epsilon) + "sfc-tmp.mp4", writer="ffmpeg")
  save_dir = "sfc-tmp" + str(epsilon) + "_"+ str(datetime.date.today())
  os.makedirs(save_dir, exist_ok=True)
